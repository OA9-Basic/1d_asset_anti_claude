...
export async function GET(req: NextRequest) {
  try {
    ...
    const stats = {
      ...
      contributingCount: contributions.filter(
        c => c.asset?.status === 'COLLECTING'
      ).length,
    ...
    purchases.forEach((purchase) => {
      ...
      if (!existingAsset) {
        ...
        existingAsset.deliveryExpiry = purchase.deliveryExpiry ? purchase.deliveryExpiry.toISOString() : null
        existingAsset.accessCount = purchase.accessCount
        existingAsset.lastAccessedAt = purchase.lastAccessedAt ? purchase.lastAccessedAt.toISOString() : null
